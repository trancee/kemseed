...
- **0d integration E2E + negatives: DONE** — `Hmb1Handshake.kt` orchestrator wiring X25519+Shake256+SHA3-256+HKDF+Gmac+ML-KEM-512 per #08 §3/§6 (ADR-0002 §O3 frame/IV/AAD conventions). `Hmb1HandshakeTest.kt` (14 tests): epk/GMAC-key/wire-TAG/transcript/ss/K_seed/(d,z,m) pinned to a Python `cryptography`+`hashlib` oracle (X25519 clamping ≅ RFC 7748); both peers reproduce the identical key tree; `Decaps_internal(sk,c*)==K_df`; 4 negative gates green — bit-flip → `GMAC_FAIL` (before DH), replay → `REPLAY` (#15 cache), A2-as-A1 reflection → `GMAC_FAIL`, all-zero `ss` → `SS_ZERO` abort before ML-KEM. Full gate: `:testAndroidHostTest` **57/57 green** + `:compileKotlinIos` green; zero new deps. Remediation `9aafefe fix(review)` (post code-review of 144e3cb): ReplayCache 2^10+30s-TTL+LRU (monotonic clock; LRU==FIFO here), `version==1` gate → `BAD_FORMAT`, `runHandshake` enforces GMAC(A1)→GMAC(A2;transcript)→derive (tampered A1 short-circuits before DH/ML-KEM), #13 `selfEncapKdf()` sole-Encaps wall, `Direction`/`WireFrame` refactor, `Derived.cStar` private behind test-only `decapsSelfCheck()`; re-review **8/8 RESOLVED**. `:testAndroidHostTest` **62/62 green** + `:compileKotlinIos` green; zero new deps.
