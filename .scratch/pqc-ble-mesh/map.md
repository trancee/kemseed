...
- **0b-preq Keccak (FIPS 202 SHA3-256 + SHA3-512 + SHAKE-128 + SHAKE-256): DONE** (red→green; 13/13 NIST-FIPS-202 known-answer vectors incl. multi-block squeeze + empty-input pad10*; base gate GREEN incl. `compileKotlinIos`). SHA3-512 added this turn as the ML-KEM KeyGen `G = d ↦ (ρ ‖ σ̂)` hash (FIPS 203 §7.1). Foundation for 0b ML-KEM GFU (`G=SHA3-512`, `H=SHA3-256`, `J=SHAKE-128`, `K=SHAKE-256`, XOF=`SHAKE-128`) + E1′ `m`/`(d,z)`/`T` derivations. CT by construction (24 fixed rounds, data-independent ρ offsets, no table lookups).
- **0b ML-KEM-512: BLOCKED on a secure q=8380417 FIPS-203 KAT oracle** — see `issues/19 §Blocker`. (pq-crystals/kyber & PQClean `ml-kem-512` default branches ship `KYBER_Q = 3329`, i.e. not FIPS-203 ML-KEM; NIST KAT relocated to a 404 "PQC Archive"; no Go/Rust/BouncyCastle/local-libcrypto to generate one.) Scaffolding + NTT/polyvec/GFU impl deferred until a q=8380417 KAT is sourced.
- **0c AES-256-GMAC + HKDF: pending** (NIST CAVP KAT; SHA256 needed for HKDF). Self-oracleable via CPython `hashlib`/`hmac` + `openssl` — can run ahead of 0b if reordered.
