...
Status: **0a DONE** (RFC 7748 §5.2 exact, 4/4 host tests pass, base gate GREEN). **0b-prereq (Keccak: SHA3-256 + SHA3-512 + SHAKE-128 + SHAKE-256, NIST FIPS-202 KAT) DONE** (13/13 host tests pass incl. new SHA3-512; base gate GREEN incl. `compileKotlinIos`). **0b (ML-KEM-512 NTT/poly/GFU): DONE** — Pure-Kotlin ML-KEM-512 green, byte-exact vs NIST FIPS-203 deterministic KAT (**q=3329, NOT 8380417**). 6/6 KAT host tests + 4 property tests + `compileKotlinIos` all GREEN; base gate preserved; zero external deps. **0c (AES-256 block + AES-256-GMAC + HKDF-SHA3-256): DONE** — AES-256 block committed `64bdee3` (FIPS-197 C.3 KAT, 3 tests); GMAC committed `f5f28e7` (GHASH = NIST SP 800-38D Alg. 2: rightshift + `0xE1` reduction into byte 0, MSB-first scan, over `Aes256.encryptBlock` + J0; 3 GMAC goldens from OpenSSL-backed `cryptography.AESGCM` oracle + 400 random-GCM + 2000-pair GHASH single-mul equivalence, 5 GmacTest tests; GMAC IV is contiguous `00..0b` (0x0a0b), NOT the `…1011` typo — cf. ADR-0002 §O3); HKDF committed `e291098` (HMAC-SHA3-256 block=136, NULL salt→32 zero bytes RFC 5869 §2.2, 255·32 clamp §2.3; 8 HkdfTest tests over RFC 5869 §B TC1/2/3 SHA3-256 variants, goldens = hand-rolled ≡ `cryptography.HKDF(SHA3_256)`). Full gate `:testAndroidHostTest` **43/43 green** + `compileKotlinIos` green; zero new runtime deps. 0d (integration E2E + negative gates): DONE — `Hmb1Handshake` orchestrator (`src/commonMain/.../Hmb1Handshake.kt`) wiring X25519+Shake256+HKDF+AES-256-GMAC+ML-KEM-512 per #08 §3/§6; `Hmb1HandshakeTest` **14 tests** (epk/GMAC-key/wire-TAG/T/ss/K_seed/(d,z,m) pinned to a Python `cryptography`+`hashlib` oracle — X25519 clamping ≅ RFC 7748; both peers reproduce the identical key tree; `Decaps_internal(sk,c*)==K_df`; 4 negative gates: bit-flip→GMAC_FAIL before DH, replay→REPLAY #15 cache, A2-as-A1 reflection→GMAC_FAIL, all-zero ss→SS_ZERO abort before ML-KEM). Full gate `:testAndroidHostTest` **57/57 green** + `compileKotlinIos` green; zero new runtime deps. Committed `144e3cb` `feat(0d): HMB1-E1' handshake orchestrator + integration/negative gates`.
...
## Result — Phase 0d (integration E2E + negative gates)
- TDD: `Hmb1Handshake.kt` stub → `Hmb1HandshakeTest.kt` (14 tests, RED — 13 failures) → implement → **GREEN**.
- **0d goldens are externally oracle-backed** (Python `cryptography` 50.0.0 + stdlib `hashlib`): epk (X25519 clamping ≅ RFC 7748), `K_gmac = HKDF-Extract("hmb1-gmac-v1", NetKey) = 7ec4…61c96`, wire MACs (AES-256-GMAC via `cryptography.AESGCM`), `T`, `ss`, `K_seed`, `(d,z)`, `m` — see test header for fixed fixtures (`NetKey=00..1f`, `ephPrivI=10..2f`, `ephPrivR=30..4f`, nonces/ids/sid, `base9=09‖00…00`); `macA1`/`macA2`/`wireA1`/`wireA2` (60 B), `T=34e820e3…`, `ss=6e3c33b4…`, `K_seed=a5bacada…`, `d=56ba8507…`, `z=bb643ba2…`, `m=b70ca7d2…`. **ML-KEM-internal path (pk/sk/K_df/c*/key_AB/keyBA is deterministic but not independently oracle-able)** → pinned by (i) both peers reproduce the identical key tree and (ii) `Decaps_internal(sk, c*) == K_df`.
- Negatives GREEN: bit-flip A1 → `GMAC_FAIL` **before** X25519/ML-KEM; replay (dup `dir‖nonce`) → `REPLAY` (#15 cache); A2-as-A1 reflection → `GMAC_FAIL` (GCM IV dir-byte 0xA0≠0xA1 mismatch); all-zero `ss` → `SS_ZERO` CT-abort before ML-KEM.
- **Gate:** `gradle testAndroidHostTest compileKotlinIos --rerun-tasks` → full suite **57/57 green** + `:compileKotlinIos` BUILD SUCCESSFUL. Committed `144e3cb` `feat(0d): HMB1-E1' handshake orchestrator + integration/negative gates`.
